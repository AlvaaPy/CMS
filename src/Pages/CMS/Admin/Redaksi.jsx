...
function Redaksi() {
  const [topikRedaksi, setTopikRedaksi] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/redaksi');
        setTopikRedaksi(response.data);
        setLoading(false);
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/redaksi/${id}`);
      setTopikRedaksi(topikRedaksi.filter((item) => item.id !== id));
    } catch (error) {
  ...
  const handleModalSubmit = async (data) => {
    try {
      if (modalData) {
        ...
        await axios.put(`http://localhost:5000/redaksi/${modalData.id}`, data);
        setTopikRedaksi(
          topikRedaksi.map((item) =>
            item.id === modalData.id ? { ...item, ...data } : item
          )
        );
      } else {
        // Create new record
        const response = await axios.post('http://localhost:5000/redaksi', data);
        setTopikRedaksi([...topikRedaksi, response.data]);
      }
      ...
      <div className="min-h-screen bg-gray-100 p-6">
        <div className="max-w-5xl mx-auto bg-white shadow-md rounded-lg">
          <div className="flex justify-between items-center p-6 border-b">
            <h1 className="text-2xl font-bold">Divisi Redaksi</h1>
            <button
              onClick={handleAdd}
