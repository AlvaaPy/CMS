...
function IT() {
  const [topikIT, setTopikIT] = useState([]);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/it/${id}`);
      setTopikIT(topikIT.filter((item) => item.id !== id));
    } catch (error) {
  ...
  const handleModalSubmit = async (data) => {
    try {
      if (modalData) {
        ...
        await axios.put(`http://localhost:5000/it/${modalData.id}`, data);
        setTopikIT(
          topikIT.map((item) =>
        ...
        // Create new record
        const response = await axios.post('http://localhost:5000/it', data);
        setTopikIT([...topikIT, response.data]);
      }
      ...
      <div className="min-h-screen bg-gray-100 p-6">
        <div className="max-w-5xl mx-auto bg-white shadow-md rounded-lg">
          <div className="flex justify-between items-center p-6 border-b">
            <h1 className="text-2xl font-bold">Divisi IT</h1>
            <button
              onClick={handleAdd}
