...
function Marketing() {
  const [topikMarketing, setTopikMarketing] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/marketing');
        setTopikMarketing(response.data);
        setLoading(false);
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/marketing/${id}`);
      setTopikMarketing(topikMarketing.filter((item) => item.id !== id));
    } catch (error) {
  ...
  const handleModalSubmit = async (data) => {
    try {
      if (modalData) {
        ...
        await axios.put(`http://localhost:5000/marketing/${modalData.id}`, data);
        setTopikMarketing(
          topikMarketing.map((item) =>
            item.id === modalData.id ? { ...item, ...data } : item
          )
        );
      } else {
        // Create new record
        const response = await axios.post('http://localhost:5000/marketing', data);
        setTopikMarketing([...topikMarketing, response.data]);
      }
      ...
      <div className="min-h-screen bg-gray-100 p-6">
        <div className="max-w-5xl mx-auto bg-white shadow-md rounded-lg">
          <div className="flex justify-between items-center p-6 border-b">
            <h1 className="text-2xl font-bold">Divisi Marketing</h1>
            <button
              onClick={handleAdd}
